...
namespace ft {
	...
	public:
		...
		map(InputIterator first, InputIterator last, const key_compare& comp = key_compare(), const allocator_type& alloc = allocator_type()) 
				:  _tree(alloc, value_compare(comp)), _alloc(alloc), _comp_func(comp) { insert(first, last); };
		...
		//  ------------  OBSERVERS   ------------
		value_compare value_comp() const { return value_compare(_comp_func); };
	
		//  ------------  OPERATIONS   ------------
		iterator find(const key_type& k) { return iterator(_tree.findNode(ft::make_pair(k, mapped_type())), &_tree); };

		const_iterator find(const key_type& k) const { return const_iterator(_tree.findNode(ft::make_pair(k, mapped_type())), &_tree); };
		...
		void	printTree() { _tree.printMapStructure(); };
	...
	private:
		...
		key_type	getKey(const_iterator it) const { return it->first; }
